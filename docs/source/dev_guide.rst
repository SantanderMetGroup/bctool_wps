...
First activate the ``bctool_wps`` Conda environment and install ``pytest``.
...
.. code-block:: console
  ...
  $ make clean
  $ make install
  $ conda list -n bctool_wps --explicit > spec-file.txt
...
Make a new version of bctool_wps in the following steps:
